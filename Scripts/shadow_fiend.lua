...
ScriptConfig:SetName("Shadow Fiend")
		...
		if not Animations.CanMove(me) and victim and GetDistance2D(me,victim) <= 2000 then
			if tick > castsleep then
				if not Animations.isAttacking(me) and victim.alive and victim.visible then
					...
					local butterfly, mom, satanic, bkb, distance = me:FindItem("item_butterfly"), me:FindItem("item_mask_of_madness"), me:FindItem("item_satanic"), me:FindItem("item_black_king_bar"), GetDistance2D(victim,me)
					if not shadowplay then
						if (math.max(math.abs(FindAngleR(me) - math.rad(FindAngleBetween(me, victim))) - 0.1, 0)) == 0 then
